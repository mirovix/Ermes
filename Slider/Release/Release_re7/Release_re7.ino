...
// Caso Start3
int AttesaStart3 =     5200;          // tempo di attesa per inizio preposizionamento dopo l'arrivo del comando START
...
// Caso Start4
int AttesaStart4 =     4000;          // tempo di attesa dopo per il rilascio magnetico dopo preposizionamento
...
void loop() {
  ...
  // MANOVRA DI RILASCIO 1 = H --> P2.1 --> EM --> P2.2 --> P1  ------------- stessa per entrambi gli slider     
  ...
  // MANOVRA DI RILASCIO 2 = H --> P1 --> P2.1 --> EM --> P2.2 --> P1         
  ...
  // MANOVRA DI RILASCIO 3 = H --> P2 --> EM --> P1      
  ...
  // MANOVRA DI RILASCIO4 = Chaser Manovra di Rilascio 2; Target Manovra di Rilascio 3       
  if (AZ_ST4 && (dt_st >= AttesaStart4)){  // Se il comando START4 è stato ricevuto e sono passati 'AttesaStart4' secondi
    ...
    // Prepos CHASER
    digitalWrite(PIN_POS1_1, HIGH);  // con HIGH apri il p-mosfet
    Serial.println("CH: Prepositioning (POS1) completed");
    // Prepos TARGET
    digitalWrite(PIN_POS2_2, HIGH);  // con HIGH apri il p-mosfet
    Serial.println("T: Prepositioning (POS2) completed");
    
    delay(AttesaPostPP2);

    // esegui la manovra
    digitalWrite(PIN_POS2_1, LOW);  // con LOW chiudi il p-mosfet
    Serial.println("CH: Initialize POS2");
