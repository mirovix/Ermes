...
// deltat variables
uint8_t pin_imu1 = 22, pin_imu2 = 24, pin_tof1 = 10, pin_tof2 = 11, pin_tof3 = 12;

//tofs values
float lux1, lux2, lux3;
uint8_t range1, range2, range3, status1, status2, status3;

unsigned long lastRefreshTime1 = 0, lastRefreshTime2 = 0, lastRefreshTime3=0, lastRefreshTime4=0, lastRefreshTime5=0, start_t1=0, start_t2=0, start_t3=0;
...
void loop() {
  ...
  //TCA9548A_PS(pin_tof1);
  tofLoop(vl1, lux1, range1, status1, name_tof1, lastRefreshTime3, start_t1);

  //loop for the second tof
  //TCA9548A_PS(pin_tof2);
  //tofLoop(vl2, lux2, range2, status2, name_tof2, lastRefreshTime4, start_t2);

  //loop for the third tof
  //TCA9548A_PS(pin_tof3);
  //tofLoop(vl3, lux3, range3, status3, name_tof3, lastRefreshTime5, start_t3);
}

void tofLoop(Adafruit_VL6180X &vl, float &lux, uint8_t &range, uint8_t &status, char *name, unsigned long &lastRefreshTime, unsigned long &start_time){
  ...
  if (status == VL6180X_ERROR_NONE) {
    Serial.print(name); Serial.print("Range: "); Serial.println(range);
  }

  // Some error occurred, print it out!
  
  if  ((status >= VL6180X_ERROR_SYSERR_1) && (status <= VL6180X_ERROR_SYSERR_5)) {
    Serial.print(name); Serial.println("System error");
  }
  else if (status == VL6180X_ERROR_ECEFAIL) {
    Serial.print(name); Serial.println("ECE failure");
  }
  else if (status == VL6180X_ERROR_NOCONVERGE) {
    Serial.print(name); Serial.println("No convergence");
  }
  else if (status == VL6180X_ERROR_RANGEIGNORE) {
    Serial.print(name); Serial.println("Ignoring range");
  }
  else if (status == VL6180X_ERROR_SNR) {
    Serial.print(name); Serial.println("Signal/Noise error");
  }
  else if (status == VL6180X_ERROR_RAWUFLOW) {
    Serial.print(name); Serial.println("Raw reading underflow");
  }
  else if (status == VL6180X_ERROR_RAWOFLOW) {
    Serial.print(name); Serial.println("Raw reading overflow");
  }
  else if (status == VL6180X_ERROR_RANGEUFLOW) {
    Serial.print(name); Serial.println("Range reading underflow");
  }
  else if (status == VL6180X_ERROR_RANGEOFLOW) {
    Serial.print(name); Serial.println("Range reading overflow");
  }
}
