...
void loop() {
  ...
  TCA9548A_PS(pin_tof1);
  tofLoop(vl1, lux1, range1, status1, name_tof1);

  //loop for the second tof
  TCA9548A_PS(pin_tof2);
  tofLoop(vl2, lux2, range2, status2, name_tof2);

  //loop for the third tof
  TCA9548A_PS(pin_tof3);
  tofLoop(vl3, lux3, range3, status3, name_tof3);
}

void tofLoop(Adafruit_VL6180X &vl, float &lux, uint8_t &range, uint8_t &status, char *name){
