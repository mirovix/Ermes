...
//{
	...
	//#ifdef VISUALIZE
		...
		////cv::namedWindow("cos(rdst", CV_WINDOW_AUTOSIZE);
...
void CAMERA::loadWorldPts( TooN::Matrix<5,3>& P )
{
	...
	// coordinate target plastica (invertite)
	//// first pt
	//p0x = 8.0;
	//p0y = -29.0;
	//p0z = 0.0;
...
//{
	...
	//#ifdef DEBUG_PERFORMANCE
		...
		////cout << "fps = " << (double)counter/getTimeElapsed(time_elapsed) << endl;
			...
			if( contours.size() == N_BLOBS )
			{
				...
				if (!isOutOfFrame()){
					...
					for (size_t i=0; i<contours.size(); i++){
						...
						//cv::circle(rgb, mc[0], 3, clr[0], -1);
						//cv::circle(rgb, mc[1], 3, clr[1], -1);
						//cv::circle(rgb, mc[2], 3, clr[2], -1);
						//cv::circle(rgb, mc[3], 3, clr[3], -1);
						//cv::circle(rgb, mc[4], 3, clr[4], -1);
					}
					imshow("rgb", rgb);
					waitKey(1);
