...
// ---------------------------------------------------------------------
CAMERA::CAMERA(){
	double valS = sqrt(2.0)/2.0;
	R_CI[0][0] = valS;//-cos(rad);
	R_CI[0][1] = valS;//-sin(rad);
	R_CI[0][2] = 0.0;
	R_CI[1][0] = -valS;//-sin(rad);
	R_CI[1][1] = valS;//cos(rad);
	R_CI[1][2] = 0.0;
	R_CI[2][0] = 0.0;
	R_CI[2][1] = 0.0;
	R_CI[2][2] = 1;//-1.0;
...
*/
	...
	*/
			...
			cout << "============================================== MV ==========" << endl;
			// first pt
			p0x = -26.1630;//26.16;//-14.85;
			p0y = -14.84;//14.85;//26.16;
			p0z = 0.0;
			// second pt
			p1x = 15.55;//-15.5;//26.87;
			p1y = 26.87;//-26.87;//-15.5;
			p1z = 0.0;
			// third pt
			p2x = 12.72;//-12.7;//-29.69;
			p2y = -29.69;//29.7;//-12.7;
			p2z = 0.0;
			// fourth pt
			p3x = -29.69;//29.7;//12.7;
			p3y = 12.72;//-12.7;//29.69;
			p3z = 0.0;
			//fifth point
			p4x = -8.48;//8.48;//2.82;
			p4y = 2.82;//-2.82;//8.48;
...
void CAMERA::setRelativeAttitudeInIMUFrame() {
	TooN::Matrix<3> R_PHI;
	R_PHI[0][0] = 1.0;//-cos(rad);
	R_PHI[0][1] = 0.0;//-cos(rad);
	R_PHI[0][2] = 0.0;//-cos(rad);
	R_PHI[1][0] = 0.0;//-cos(rad);
	R_PHI[1][1] = -1.0;//-cos(rad);
	R_PHI[1][2] = 0.0;//-cos(rad);
	R_PHI[2][0] = 0.0;//-cos(rad);
	R_PHI[2][1] = 0.0;//-cos(rad);
	R_PHI[2][2] = -1.0;//-cos(rad);
	
	rel_att_in_imu_f = R_PHI * R_CI * sol.slice<0,0,3,3>();
	...
	cout << "\t\t\trel_state_imu_frame:\t" << RAD2DEG * rel_state[3] << ", " 
											<< RAD2DEG * rel_state[4] << ", " 
											<< RAD2DEG * rel_state[5] << endl;
	/*
	...
	cout << "QUAT: " << quaternion[0]<< ", "<<quaternion[1]<< ", "<<quaternion[1]<< ", "<<quaternion[2]<<", "quaternion[3] << endl;
*/
...
void CAMERA::setRelState(void)
{
	...
	printf("\t\t\trel_state_cam_frame:\t[%f, %f, %f]\n", RAD2DEG * rel_state[3], 
														RAD2DEG * rel_state[4], 
														RAD2DEG * rel_state[5]);
}
...
void CAMERA::sortBlobs(void)
{
	...
	if(isFirstFrame || !patternInitialized)
	{
...
{
	save_flag = true; 
	snprintf(rslt_file_name, FILENAME_SIZE, "pacman/results/TestCAM_%d__%s.dat"/*"../results/TestCAM_%d__%s.dat"*/,test_number,currentDateTime().c_str());
...
void CAMERA::SaveData(void)
{
	...
	*/
	...
	if(*camCommunicationON) {
		...
		filt->measurementUpdate(quaternion, 1000.0 * getTimeElapsed(time_elapsed));
	}
}
			...
			if( contours.size() == N_BLOBS )
			{
				...
				if (!isOutOfFrame()){
					...
					if(err_param < threshold_error)	 {
						...
						setRelativeAttitudeInIMUFrame(); // rotate attitude in imu ref frame
						...
						patternInitialized = false;
						cout << "Soluzione saltata, err: " << err_param << endl;
						// reinizializzo i blobs
						sortBlobs();
