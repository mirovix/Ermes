...
static void CCONV onAttachHandler(PhidgetHandle phid, void *ctx) {
	...
	snprintf(rslt_file_name, FN_SIZE-1, "pacman/results/TestIMU_%d__%s.dat"/*"../results/TestIMU_%d__%s.dat"*/, test_number ,currentDateTime().c_str());
						...
						double timestamp)
{
	...
	if(*imuCommunicationON) {
	 	...
	 	filt->timeUpdate(angleVels, 0.012, timeG);
	 	//printf("\tdopo\n");
	}
