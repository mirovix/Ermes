...
#define SZ 80 // length of the file name string
...
public:
	...
	void timeUpdate(float *imuMeasurement, float dt, double curr_t);
	void measurementUpdate(float *camMeasurement, double curr_t);
	void setQ(float **Q);
	void setR(float **R);
...
private:
	...
	FILE* file;
	char rslt_file_name[SZ];

	void saveData(double curr_t, char id);
	...
	mutex filtMut;
	bool firstMeasureFromCamera = false; // <-------------------------------  DEVE ESSERE FALSE
};
#endif
