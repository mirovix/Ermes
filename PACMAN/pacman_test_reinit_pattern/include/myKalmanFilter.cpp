...
void MyKalmanFilter::timeUpdate(float *imuMeasurement, float dt, double curr_t) {
...
void MyKalmanFilter::saveData(double curr_t, char  id){
		fprintf(file, "%c\t%.0f\t%.3f\t%.3f\n",id, curr_t, angDeg[0], angDeg[1]);
}

void MyKalmanFilter::EnableDataSaving(void)
{
	snprintf(rslt_file_name, SZ, "pacman/results/TestKF_%d__%s.dat"/*"../results/TestCAM_%d__%s.dat"*/,test_number,currentDateTime().c_str());
...
const string MyKalmanFilter::currentDateTime(void)
{
	time_t     now = time(0);
	struct tm tstruct;
	char      buf[80];
...
void MyKalmanFilter::measurementUpdate(float * camMeasurement, double curr_t) {
	filtMut.lock();
